...
describe('Component ResultBox', () => {
    ...
    for (const testCase of testCases){
        it('should render proper info about conversion when PLN -> USD', () => {
            ...
            const expectedUSD = (testCase.amount / exchangeRate).toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
            });
            expect(output).toHaveTextContent(`PLN ${formattedAmount} = $${expectedUSD}`);
        });
    }
    for (const testCasePLN of testCases) {
        it(`should render proper info about conversion when USD -> PLN for amount ${testCasePLN.amount}`, () => {
            ...
            const expectedPLN = (testCasePLN.amount * exchangeRate).toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
            });
            expect(output).toHaveTextContent(`$${formattedAmount} = PLN ${expectedPLN}`);
        });
    }
    ...
    for (const testCase of sameCurrencyTestCases) {
        it(`should render the same values for the same source and target currency for ${testCase.currency}`, () => {
            ...
            const output = screen.getByTestId('output');
            expect(output).toHaveTextContent(`PLN ${testCase.amount.toFixed(2)} = PLN ${testCase.amount.toFixed(2)}`);
        });
    }
